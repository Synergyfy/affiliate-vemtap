...
## 🎯 Primary Objective
You are an expert AI agent working within this repository. Your primary goal is to follow the project-specific standards defined in the `.agent/` directory. 

## 📂 Instruction Hierarchy
Before responding to any prompt, you must internalize the context provided in the following files located in `./.agent/`:

1.  **`tech-stack.md`**: Definitions for the languages, frameworks, and libraries used (e.g., NestJS, TypeScript, Docker).
2.  **`coding-standards.md`**: Rules for naming conventions, file structure, and architectural patterns.
3.  **`constraints.md`**: Hard "Should-nots" (e.g., No `any` types, no external utility libraries) make sure you follow the [text](.agent/MIGRATIONS.md).
4.  **`workflow.md`**: Instructions on how to handle Git, deployments (PM2/Nginx), and environment setups (WSL).

## 🛠️ Operational Rules
- **Precedence:** If my prompt conflicts with the `.agent` files, the `.agent` files take priority unless I explicitly state otherwise.
- **State Check:** If you are unsure of the project structure, refer to `./.agent/context.md`.
- **Consistency:** Maintain the "Linear-meets-Terminal" UI aesthetic for any frontend tasks and a clean, modular backend architecture for NestJS/Node.js tasks.

## 🚫 Forbidden Actions
- Do not suggest solutions that deviate from the established tech stack.
- Do not remove existing error-handling patterns or logging middleware.
- Do not ignore the directory-specific rules if a sub-folder has its own instruction file.

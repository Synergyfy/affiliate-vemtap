...
export class ApiKeysService {
  ...
  async generate(
    ...
    this.logger.log(
      `API key "${dto.name}" generated by admin ${adminId} (prefix: ${prefix})`,
    );
  ...
  async validateKey(rawKey: string): Promise<void> {
    ...
    if (!matchedId) {
      this.logger.warn(
        `Invalid API key attempt (prefix: ${rawKey.substring(0, 8)}...)`,
      );
    ...
    this.prisma.apiKey
      ...
      .catch((err: Error) =>
        this.logger.error("Failed to update lastUsedAt", err.stack),
      );
  }
  ...
  async revoke(id: string, adminId: string): Promise<void> {
    ...
    this.logger.log(
      `API key "${key.name}" (${key.prefix}...) revoked by admin ${adminId}`,
    );
  }
}

...
describe('AuthService', () => {
  ...
  describe('refreshTokens', () => {
    it('should issue new tokens if refresh token is valid', async () => {
      ...
      mockJwtService.verifyAsync.mockResolvedValueOnce({ sub: '1', tokenVersion: 1 });
    ...
    it('should throw UnauthorizedException if tokenVersion mismatch', async () => {
      ...
      mockJwtService.verifyAsync.mockResolvedValueOnce({ sub: '1', tokenVersion: 1 }); // Token version is 1
      mockUsersService.findById.mockResolvedValueOnce(mockUser);

      await expect(service.refreshTokens('1', 'old-refresh')).rejects.toThrow(UnauthorizedException);
    });
  });
});

              ...
              <div className="flex items-center justify-between">
                ...
                {formData.gpsLat && formData.gpsLng && (
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                    <Navigation className="w-3 h-3 text-emerald-600 fill-emerald-600/20" /> GPS Location Attached
                  </span>
                )}
              </div>
            </div>
              ...
              {activeTab === 'sales' && (
                <div className="space-y-4">
                  ...
                  <div className="pt-2 border-t border-slate-100">
                    ...
                    {/* Saved GPS — shown when coordinates already exist */}
                    {formData.gpsLat && formData.gpsLng ? (
                      <div className="space-y-2">
                        <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-2xl space-y-2">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-xl bg-emerald-100 flex items-center justify-center shrink-0">
                              <Navigation className="w-4 h-4 text-emerald-600 fill-emerald-200" />
                            </div>
                            <div className="min-w-0 flex-1">
                              <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700">GPS Location Saved</p>
                              <p className="text-[11px] font-bold text-emerald-900 font-mono mt-0.5">
                                {formData.gpsLat}, {formData.gpsLng}
                              </p>
                            </div>
                            <span className="text-[9px] font-black px-2 py-1 bg-emerald-200 text-emerald-800 rounded-lg shrink-0">✓ SET</span>
                          </div>
                          {formData.gpsAddress && (
                            <div className="flex items-start gap-1.5 pt-1.5 border-t border-emerald-200/60">
                              ...
                              <p className="text-[10px] text-emerald-700 leading-snug font-medium">{formData.gpsAddress}</p>
                            </div>
                          )}
                        </div>
                        <button
                          ...
                          className="w-full py-2.5 rounded-xl border border-emerald-300 bg-white hover:bg-emerald-50 text-emerald-700 text-xs font-bold flex items-center justify-center gap-2 transition-all active:scale-[0.99]"
                        >
                      ...
                      /* No GPS saved yet — show capture button */
                      <button
                        type="button"
                        onClick={async () => {
                          if (navigator.geolocation) {
                        ...
                        className="w-full py-4 rounded-xl border-2 border-blue-600 bg-blue-600 hover:bg-blue-700 flex flex-col items-center justify-center gap-1.5 transition-all active:scale-[0.99]"
                      >
                        <Navigation className="w-6 h-6 text-white" />
                        <span className="text-sm font-bold text-white">Capture GPS Location</span>
                        <span className="text-[10px] font-medium text-blue-100">Tap to record your exact location</span>
                      </button>
                    )}
                  </div>
                </div>

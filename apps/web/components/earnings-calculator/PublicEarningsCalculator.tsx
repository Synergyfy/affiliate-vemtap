...
import {
  Calculator, Info, BookOpen, Percent, RefreshCw, Target,
...
// Standard public commission rates (affiliate perspective)
const DIRECT_RATE = 20;
const INDIRECT_RATE = 5;
const RECURRING_AFFILIATE_RATE = 10;
const RECURRING_DURATION = 12;
...
interface PublicEarningsCalculatorProps {
  header?: ReactNode;
}

export default function PublicEarningsCalculator({ header }: PublicEarningsCalculatorProps) {
  const searchParams = useSearchParams();

  const [tab, setTab] = useState<'calculator' | 'guide'>('calculator');
  const [target, setTarget] = useState<number>(DEFAULT_DAILY_TARGET);
  const [planCounts, setPlanCounts] = useState<Counts>(() => defaultDistribution(DEFAULT_DAILY_TARGET));
  ...
  const perPlan = useMemo(() => {
    return SUBSCRIPTION_PLANS.map(plan => {
      ...
      return {
        ...
        subValue,
        directMonthly: subValue * (DIRECT_RATE / 100),
        recurringMonthly: subValue * (RECURRING_AFFILIATE_RATE / 100),
      };
    });
  }, [planCounts]);

  const directMonthly = subscriptionMonthly * (DIRECT_RATE / 100);
  const recurringMonthly = subscriptionMonthly * (RECURRING_AFFILIATE_RATE / 100);
  const indirectMonthly = subscriptionMonthly * (INDIRECT_RATE / 100);
  const recurringIndirectMonthly = subscriptionMonthly * (0.03);
  const recurringMonths = Math.max(0, RECURRING_DURATION - 1);
  ...
  const projection = useMemo(() => {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      const total = direct + recurring;
      cumulative += total;
      months.push({ month: m, direct, recurring, indirect: 0, total, cumulative });
    }
    return months;
  }, [directMonthly, recurringMonthly, recurringMonths]);
  ...
  const resetCalculator = () => {
    setCustomized(false);
    setTarget(DEFAULT_DAILY_TARGET);
    setPlanCounts(defaultDistribution(DEFAULT_DAILY_TARGET));
  };
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 rounded-3xl p-6 text-white space-y-5 relative overflow-hidden">
            ...
            <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className="bg-white/5 rounded-xl p-3 border border-white/10">
                <p className="text-[9px] font-bold text-white/50 uppercase tracking-wider">Recurring total (mo 2–{RECURRING_DURATION})</p>
          ...
          <p className="text-[10px] text-slate-400 leading-relaxed border-t border-slate-100 pt-4">
            Assumes {DIRECT_RATE}% direct commission, {RECURRING_AFFILIATE_RATE}% monthly recurring for {RECURRING_DURATION} months, based on the standard affiliate program rates. Sign up to see the exact rates applied to your account.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            ...
            <p className="text-xs text-slate-600 leading-relaxed mb-3">
              You earn <span className="font-bold text-emerald-600">{DIRECT_RATE}%</span> on every business you personally bring to Vemtap that subscribes to a paid plan. This is your <span className="font-bold">direct</span> commission — you get it on the first month&apos;s subscription payment of each customer you close.
            </p>
            <div className="bg-slate-50 rounded-xl p-3 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-600">Rate</span>
              <span className="text-sm font-black text-emerald-600">{DIRECT_RATE}% of subscription</span>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-black text-slate-900">Recurring Commission (Affiliate)</h4>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed mb-3">
              When a business you brought in <span className="font-bold">renews</span> its subscription each month, you continue earning a recurring <span className="font-bold text-blue-600">{RECURRING_AFFILIATE_RATE}%</span> commission for up to {RECURRING_DURATION} months. This is what makes building a portfolio of active businesses so valuable.
            </p>
            <div className="bg-slate-50 rounded-xl p-3 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-600">Recurring rate</span>
              <span className="text-sm font-black text-blue-600">{RECURRING_AFFILIATE_RATE}% / month for {RECURRING_DURATION} months</span>
            </div>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-sm font-black text-slate-900">Indirect / Team Commission</h4>
            </div>
            <p className="text-xs text-slate-600 leading-relaxed mb-3">
              Recruit and build a team to unlock <span className="font-bold text-amber-600">{INDIRECT_RATE}%</span> indirect commission from your sub-affiliates&apos; sales. As your network grows, you earn on their success too.
            </p>
            <div className="bg-slate-50 rounded-xl p-3 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-600">Indirect rate</span>
              <span className="text-sm font-black text-amber-600">{INDIRECT_RATE}% of network sales</span>
            </div>
          </div>
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex gap-3">
            <Info className="w-5 h-5 text-blue-600 shrink-0" />
            <p className="text-xs text-blue-800 leading-relaxed">
              These are the standard program rates. Sign up for free to see the exact rates applied to your account in your dashboard.
            </p>
          </div>
        </div>
      )}

...
import { X, CheckCircle2, AlertCircle, Calendar, MessageSquare } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useToast } from '@/hooks/use-toast';
import { useCompleteFollowUp } from '@/services/useSalesPipeline';
import { useUpdatePipelineStage } from '@/services/useSalesPipeline';
import { FollowUpOutcome, SalesPipelineStage } from '@/types/sales-pipeline';
...
const OUTCOME_OPTIONS = [
  { value: 'INTERESTED', label: 'Still Interested', icon: CheckCircle2, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200' },
  { value: 'NEEDS_INFO', label: 'Needs More Info', icon: AlertCircle, color: 'text-blue-700', bg: 'bg-blue-50 border-blue-200' },
  { value: 'DEMO_REQUESTED', label: 'Demo Requested', icon: Calendar, color: 'text-indigo-700', bg: 'bg-indigo-50 border-indigo-200' },
  { value: 'NOT_INTERESTED', label: 'Not Interested', icon: AlertCircle, color: 'text-red-700', bg: 'bg-red-50 border-red-200' },
  { value: 'RESCHEDULE', label: 'Reschedule', icon: MessageSquare, color: 'text-amber-700', bg: 'bg-amber-50 border-amber-200' },
  { value: 'CONVERTED', label: 'Converted', icon: CheckCircle2, color: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200' },
  { value: 'LOST', label: 'Lost', icon: AlertCircle, color: 'text-slate-700', bg: 'bg-slate-100 border-slate-200' },
] as const;
...
export default function FollowUpCompletionModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (outcome === 'DEMO_REQUESTED') {
        ...
        showToast('Follow-up completed: ' + outcome.replace('_', ' '), 'success');
      } else {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end">
      ...
      <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="relative bg-white w-full max-w-md mx-auto mb-safe rounded-t-3xl p-6 shadow-2xl max-h-[85vh] overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Add any notes about this follow-up..." rows={3} className="w-full mt-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium resize-none focus:outline-none focus:ring-2 focus:ring-blue-100" />
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={handleSubmit} disabled={!outcome || isSubmitting} className="flex-1 py-3.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-black text-sm disabled:opacity-50 flex items-center justify-center gap-2">
            {isSubmitting ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><CheckCircle2 className="w-4 h-4" />Complete Follow-up</>}
          </button>
        </div>
      </motion.div>
    </div>

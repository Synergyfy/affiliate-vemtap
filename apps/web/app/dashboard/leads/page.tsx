...
const initialLeads = [
  { id: 1, businessName: 'Nexus Retail Group', contact: 'David Chen', industry: 'Retail', status: 'Potential', date: '2026-05-12', time: '10:30 AM', phone: '+234 801 234 5678', location: 'Lagos, Nigeria', website: 'www.nexusretail.com', comments: 'Very interested in our POS solution.', reason: '' },
  { id: 2, businessName: 'Stellar Tech Corp', contact: 'Sarah Miller', industry: 'Tech', status: 'Contacted', date: '2026-05-11', time: '02:15 PM', phone: '+234 802 345 6789', location: 'Abuja, Nigeria', website: 'www.stellartech.ng', comments: 'Requested a demo for next week.', reason: '' },
  { id: 3, businessName: 'Blue Diamond Hotels', contact: 'James Wilson', industry: 'Hospitality', status: 'Interested', date: '2026-05-10', time: '09:45 AM', phone: '+234 803 456 7890', location: 'Port Harcourt, Nigeria', website: 'www.bluediamond.com', comments: 'Decision maker is out of town until Monday.', reason: '' },
  { id: 4, businessName: 'Green Valley Farms', contact: 'Emma Thompson', industry: 'Agriculture', status: 'Not Interested', date: '2026-05-09', time: '11:20 AM', phone: '+234 804 567 8901', location: '', website: '', reason: 'Already using a competitor', comments: 'Happy with current provider.' },
];
...
const statuses = [
  ...
  { id: 'Not Interested', name: 'Not Interested', color: 'bg-red-500', bg: 'bg-red-50', text: 'text-red-600' },
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filteredLeads.map((lead) => (
                      <tr key={lead.id} onClick={() => openDetails(lead)} className="group hover:bg-slate-50 transition-colors cursor-pointer">
                        ...
                        <td className="px-6 py-5">
                          <div>
                            <p className="text-sm font-bold text-slate-700">{lead.contact}</p>
                            <p className="text-xs text-slate-400">{lead.phone}</p>
                          </div>
                        </td>
                        <td className="px-6 py-5 text-center">
                          <div onClick={(e) => e.stopPropagation()}>
                            <select value={lead.status} onChange={(e) => handleStatusChange(lead.id, e.target.value)} className={cn("inline-block px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest outline-none border-transparent cursor-pointer shadow-sm", statuses.find(s => s.id === lead.status)?.bg, statuses.find(s => s.id === lead.status)?.text)}>
                        ...
                        <td className="px-6 py-5">
                          <div className="flex items-center gap-2 text-slate-500">
                            ...
                            <div><p className="text-xs font-bold">{lead.date}</p><p className="text-[10px] text-slate-400">{lead.time || '10:00 AM'}</p></div>
                          </div>
                        </td>
                        ...
                        <motion.div key={lead.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} onClick={() => openDetails(lead)} className="bg-white p-5 rounded-[24px] shadow-sm border border-slate-100 hover:shadow-xl hover:shadow-blue-50/50 transition-all group relative overflow-hidden cursor-pointer">
                          <div className="space-y-4 relative z-10">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{lead.contact} • {lead.industry}</p>
                            </div>
                            <div className="pt-4 border-t border-slate-50" onClick={(e) => e.stopPropagation()}>
                              <select value={lead.status} onChange={(e) => handleStatusChange(lead.id, e.target.value)} className={cn("w-full px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest outline-none border-transparent focus:ring-2 focus:ring-blue-100 transition-all cursor-pointer", status.bg, status.text)}>
                                {statuses.map(opt => <option key={opt.id} value={opt.id}>{opt.name}</option>)}
                              </select>
                            </div>
                          </div>

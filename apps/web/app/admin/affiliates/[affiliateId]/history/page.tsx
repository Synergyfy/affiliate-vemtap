    ...
    <AdminLayout>
      <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6">
        ...
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setSelectedLocation(null)} className={cn('px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs font-bold', !selectedLocation ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600')}><History className="w-3.5 h-3.5 inline mr-1.5" />All Locations</button>
          {locations.map(location => <button key={location.id} onClick={() => setSelectedLocation(location.cluster.id)} className={cn('px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs font-bold', selectedLocation === location.cluster.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600')}><MapPin className="w-3.5 h-3.5 inline mr-1.5" />{location.cluster.name}</button>)}
        </div>

        <div className="relative"><Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search activity..." className="w-full pl-9 pr-3 py-2 sm:py-2.5 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm" /></div>
        ...
        {entries.length === 0 ? <div className="text-center py-16"><History className="w-12 h-12 text-slate-300 mx-auto mb-3" /><p className="text-sm font-bold text-slate-500">No activity found</p><p className="text-xs text-slate-400 mt-1">There is no recorded activity matching the current filters.</p></div> :
          <div className="space-y-2">{entries.map(entry => <div key={entry.id} className="flex items-start gap-3 p-3 sm:p-4 bg-white border border-slate-100 rounded-xl">
